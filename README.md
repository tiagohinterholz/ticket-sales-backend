...
- **Convite de transferência pendente não é listável**: não existe endpoint para listar convites pendentes de um ticket — o front mostra esse estado só logo após criar o convite (state local), some se a página recarregar. O dono ainda consegue cancelar acessando `/transfers/:token` diretamente, então a funcionalidade central não é afetada, só a conveniência de visualização.
