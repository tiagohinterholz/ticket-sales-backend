...
### AD-010
- **Decision**: Repo web ganhou uma suíte de testes básica com **Vitest + React Testing Library** (`@testing-library/react` v16+, `@testing-library/jest-dom`, `@testing-library/user-event`, `jsdom`). Config vive em `vite.config.ts` (bloco `test`, via `/// <reference types="vitest/config" />` — sem duplicar a config do Vite), setup em `src/test/setup.ts` (`@testing-library/jest-dom/vitest` + `cleanup()` em `afterEach`). Testes ficam co-localizados com o componente (`ComponentName.test.tsx`), sem `globals: true` — cada teste importa `describe`/`it`/`expect`/`vi` explicitamente de `vitest`. A camada `src/api/*.ts` é mockada por módulo (`vi.mock("../../api/tickets")` etc.); os hooks de `src/api/hooks/` (AD-009) rodam de verdade por cima do mock, então a fiação hook→API também fica coberta. `QrScanner` (câmera, via `html5-qrcode`) é mockado como stub — câmera não é testável em jsdom (sem `navigator.mediaDevices`), então a cobertura da portaria é só pelo campo de digitação manual.
- **Reason**: Decisão de escopo revisitada pelo usuário — sobrou tempo até o prazo e o projeto (T01-T39 + fixes pós-Verifier) já estava com o backend 133/133 e o front verificado manualmente; "cobrir o básico" no front antes de encerrar.
- **Escopo coberto**: `CheckoutPage` (cartão terminado em `0000` → recusado com opção de retry; qualquer outro cartão aprovado → navega para o ingresso; hold com `expires_at` no passado → reserva expirada), `TicketDetailPage` (cancelamento: confirmação antes de cancelar, status exibido atualiza após sucesso, mensagem de erro da API — não genérica — em cancelamento fora da janela), `GatePage` (os 4 `GateResult` — `VALID`/`INVALID`/`ALREADY_USED`/`WRONG_EVENT` — renderizam o banner correto via digitação manual). 10 testes ao todo.
- **Escopo não coberto (deliberado)**: todas as outras telas (auth, listagem/detalhe de evento, organizador, transferência, portaria via câmera) — já verificadas manualmente contra o backend real ao longo do desenvolvimento; pedido do usuário foi cobertura básica, não suíte exaustiva. Ver `frontend/README.md` para o registro dessa fronteira.
- **Trade-off**: Sem `globals: true` significa import explícito em cada arquivo de teste (mais verboso), mas evita poluir o ambiente global de tipos do TypeScript e é consistente com `verbatimModuleSyntax` já ativo no `tsconfig.app.json`. Sem teste de câmera é uma lacuna real (fluxo de QR scan da portaria só é validado manualmente) — aceito explicitamente por não ser testável em jsdom sem infraestrutura adicional (mock de `getUserMedia`) que o pedido do usuário não justificava.
- **Scope**: Repo web (`ticket-sales-platform-web`) — qualquer teste futuro de componente segue este mesmo padrão (mock por módulo de API, hooks reais por cima, `MemoryRouter` + `QueryClientProvider` quando o componente depender deles).
- **Date**: 2026-08-14
- **Status**: active
...
- **Phase / Task**: Todas as 39 tasks (T01–T39) commitadas nos dois repos. Verifier de fim-de-feature rodou (`validation.md`), achou 2 gaps Major (GATE-02: `used_at` ausente na resposta de "já utilizado"; CATALOG-01: organizador não escolhia o filme entre resultados da busca) + 2 menores (edge case de edição de evento não documentado; teste de JWT com tamper flaky) — todos corrigidos e revalidados manualmente (testes + chamadas reais contra TMDb/Postgres) após o relatório. `validation.md` reflete o veredito da 1ª passada (FAIL com os gaps); os commits posteriores a ele contêm as correções. Sessão seguinte (2026-08-14) adicionou suíte de testes básica no front-end (AD-010) — fora do `tasks.md` original, mini-feature própria com seu Execute + Verifier.
- **Completed**: spec.md, context.md, design.md, tasks.md, STATE.md (AD-001..AD-010), os dois repos com backend (T01-T25) + frontend (T26-T35) + seed/compose/READMEs (T36-T39) + fixes pós-Verifier + suíte de testes do front (AD-010), todos commitados. Suíte do backend: 133 testes passando; suíte do front: 10 testes passando (checkout, cancelamento, portaria).
...
- **Bugs reportados pelo usuário durante esta sessão, ainda não corrigidos** (fora do escopo desta mini-feature de testes, capturados aqui para não se perder): (1) `EventOccupancyPage`/dashboard do organizador ("meus eventos") não tem botão de voltar; (2) título de filme longo (ex.: "Matrix") estoura o layout do card de evento — CSS não trata overflow/truncamento de título longo.
- **Blockers**: none
